...
void NewsChecker::reloadNews(const QString& newUrl)
{
    ...
    // Separate cache file per feed URL so switching away from Prism/Pinecone RSS does not reuse old entries.
    const QString cacheFile = QString("feed_%1.xml").arg(QString::number(qHash(m_feedUrl), 16));
    m_entry = APPLICATION->metacache()->resolveEntry("feed", cacheFile);
...
void NewsChecker::rssDownloadFinished()
{
    ...
    // If the parsing succeeded, read it (RSS 2.0 uses <item>, Atom uses <entry>).
