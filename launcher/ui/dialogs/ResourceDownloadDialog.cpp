...
ResourcePage* ResourceDownloadDialog::selectedPage()
{
    ...
    if (!result) {
        qWarning() << "ResourceDownloadDialog::selectedPage() called but no page is selected. Empty list?";
    }
    return result;
}
...
void ResourceDownloadDialog::selectedPageChanged(BasePage* previous, BasePage* selected)
{
    ...
    if (!result) {
        qWarning() << "ResourceDownloadDialog::selectedPageChanged() called with a non-ResourcePage selected!";
        return;
    }
...
void ResourceDownloadDialog::setResourceMetadata(const std::shared_ptr<Metadata::ModStruct>& meta)
{
    ...
    if (page) {
        ...
        qWarning() << "ResourceDownloadDialog::setResourceMetadata: No page selected - cannot open project" << meta->project_id;
    }
}
