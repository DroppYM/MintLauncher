...
int MSALoginDialog::exec()
{
    ...
    connect(m_authflow_task.get(), &Task::succeeded, this, &MSALoginDialog::onAuthTaskSucceeded);
    connect(m_authflow_task.get(), &Task::aborted, this, [this]() {
        if (m_authFinished) {
            return;
        }
        m_authFinished = true;
    ...
    connect(m_devicecode_task.get(), &Task::succeeded, this, &MSALoginDialog::onAuthTaskSucceeded);
    connect(m_devicecode_task.get(), &Task::aborted, this, [this]() {
        if (m_authFinished) {
            return;
        }
        m_authFinished = true;
...
void MSALoginDialog::stopAuthTasks()
{
    if (m_authflow_task) {
        disconnect(m_authflow_task.get(), nullptr, this, nullptr);
        if (m_authflow_task->isRunning()) {
            m_authflow_task->abort();
        }
    }
    if (m_devicecode_task) {
        disconnect(m_devicecode_task.get(), nullptr, this, nullptr);
        if (m_devicecode_task->isRunning()) {
            m_devicecode_task->abort();
        }
    }
}

void MSALoginDialog::onAuthTaskSucceeded()
{
    if (m_authFinished) {
        return;
    }
...
void MSALoginDialog::closeEvent(QCloseEvent* event)
{
    if (!m_authFinished) {
        m_authFinished = true;
        stopAuthTasks();
...
{
    auto authflow = m_authflow_task;
    auto devicecode = m_devicecode_task;
    stopAuthTasks();
