...
void PageContainer::showPage(int row)
{
    ...
    m_switchingPage = true;

    if (m_currentPage) {
        m_currentPage->closed();
    }
    if (row >= 0 && row < m_model->pages().size()) {
    ...
    m_switchingPage = false;
}

void PageContainer::help()
{
    Q_UNUSED(m_currentPage);
    DesktopServices::openUrl(QUrl(BuildConfig.WIKI_URL));
}

void PageContainer::currentChanged(const QModelIndex& current)
{
    QMetaObject::invokeMethod(this, [this, current]() { applyCurrentChanged(current); }, Qt::QueuedConnection);
}

void PageContainer::applyCurrentChanged(const QModelIndex& current)
{
    if (m_switchingPage) {
        return;
    }
    ...
    if (current.isValid()) {
        ...
        if (sourceIndex.isValid()) {
            selected_index = sourceIndex.row();
        }
    }

    BasePage* selected = nullptr;
    if (selected_index >= 0 && selected_index < m_model->pages().size()) {
        selected = m_model->pages().at(selected_index);
    }

    if (selected == m_currentPage) {
        return;
    }
