    ...
    , m_logSearchPaths(instance ? instance->getLogFileSearchPaths() : QStringList{ "logs" })
{
    ...
    connect(&m_watcher, &QFileSystemWatcher::directoryChanged, this, &OtherLogsPage::populateSelectLogBoxDeferred);
...
void OtherLogsPage::populateSelectLogBoxDeferred()
{
    QMetaObject::invokeMethod(this, &OtherLogsPage::populateSelectLogBox, Qt::QueuedConnection);
}

void OtherLogsPage::populateSelectLogBox()
{
    if (!m_isPageOpen || m_reloadInProgress) {
        return;
    }
    ...
    if (!m_instance) {
        ...
        ui->selectLogBox->addItem(tr("Select a log file..."));
    }
...
{
    if (m_instance && index <= 0) {
        m_currentFile = QString();
        if (ui && ui->text) {
            ui->text->setModel(nullptr);
            ui->text->clear();
        }
        setControlsEnabled(ui->selectLogBox->count() > 1);
        return;
    }
...
void OtherLogsPage::reload()
{
    ...
    if (!file.open(QFile::ReadOnly)) {
        const QString failedPath = m_currentFile;
        setControlsEnabled(false);
        ui->btnReload->setEnabled(true);  // allow reload
        m_currentFile = QString();
        QMessageBox::critical(this, tr("Error"), tr("Unable to open %1 for reading: %2").arg(failedPath, file.errorString()));
        m_reloadInProgress = false;
        m_watcher.blockSignals(false);
    } else {
        ...
        // Instance log files are static on disk; plain text avoids LogModel churn that crashed in debug builds.
        if (m_instance) {
            const int maxLines = getConsoleMaxLines(m_instance->settings());
            QString content;
            content.reserve(static_cast<int>(std::min(file.size(), qint64(512 * 1024))));
            int lineCount = 0;
            auto appendLine = [&content, &lineCount, maxLines](QString line) -> bool {
            ...
            if (file.fileName().endsWith(".gz")) {
                QString line;
                auto error = GZip::readGzFileByBlocks(&file, [&line, appendLine](const QByteArray& d) {
                    auto block = d;
                ...
                while (!file.atEnd()) {
                    if (!appendLine(QString::fromUtf8(file.readLine()))) {
                        break;
                    }
                }
            }
            if (lineCount >= maxLines) {
                content += tr("\n... log truncated at %1 lines ...\n").arg(maxLines);
            }
