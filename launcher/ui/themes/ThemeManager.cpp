...
{
    const QString id = !catName.isEmpty() ? catName : APPLICATION->settings()->get("BackgroundCat").toString();
    if (id.isEmpty()) {
        return {};
    }

    auto catIter = m_catPacks.find(id);
    ...
    themeWarningLog() << "Tried to get invalid catPack:" << id;
    return {};
}
...
{
    // Mint Launcher: no built-in background cats (users may add custom catpacks in the data folder).
