    ...
    : QAbstractListModel(parent), m_dir(dir), m_instance(instance), m_watcher(this), m_is_indexed(is_indexed)
{
    ...
    if (m_instance) {
        connect(m_instance, &BaseInstance::runningStatusChanged, this, [this](bool running) {
            if (!running && m_pending_directory_update) {
                m_pending_directory_update = false;
                update();
            }
        });
    }
...
void ResourceFolderModel::directoryChanged(QString path)
{
    ...
    // Minecraft locks and rewrites mod folders while running; live reloads race the UI.
    if (m_instance && m_instance->isRunning()) {
        m_pending_directory_update = true;
        return;
    }
...
bool ResourceFolderModel::ProxyModel::filterAcceptsRow(int source_row, [[maybe_unused]] const QModelIndex& source_parent) const
{
    ...
    if (!model || source_parent.isValid())
        return false;

    if (source_row < 0 || source_row >= model->size())
        return false;
...
bool ResourceFolderModel::ProxyModel::lessThan(const QModelIndex& source_left, const QModelIndex& source_right) const
{
    ...
    if (!model->validateIndex(source_left) || !model->validateIndex(source_right)) {
        return QSortFilterProxyModel::lessThan(source_left, source_right);
    }
