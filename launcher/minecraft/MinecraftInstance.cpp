...
LaunchTask* MinecraftInstance::createLaunchTask(AuthSessionPtr session, MinecraftTarget::Ptr targetToJoin)
{
    ...
    if (session->launchMode != LaunchMode::Offline) {
        ...
        // Mint Launcher: install CustomSkinLoader into mods on launch (not during instance creation)
        process->appendStep(makeShared<TaskStepWrapper>(pptr, makeShared<CustomSkinLoaderInstallTask>(this, mode)));
    } else {
