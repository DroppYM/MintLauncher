...
{
    // Don't fail twice (Debug builds used to Q_ASSERT here and hard-crash the launcher).
    if (!isRunning()) {
        qCWarning(taskLogC) << "Task" << describe() << "failed while not running (ignored):" << reason;
        return;
    }
...
void Task::emitAborted()
{
    // Don't abort twice.
    if (!isRunning()) {
        qCWarning(taskLogC) << "Task" << describe() << "aborted while not running (ignored)";
        return;
    }
...
void Task::emitSucceeded()
{
    // Don't succeed twice.
    if (!isRunning()) {
        qCWarning(taskLogC) << "Task" << describe() << "succeeded while not running (ignored)";
        return;
    }
